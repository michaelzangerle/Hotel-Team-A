...
public class ModelZimmer implements IModelZimmer {
	...
	public List<IZimmer> getVerfuegbareZimmerFürGegebeneKategorie(
			IKategorie ausgewählteKategorie,IReservierung aktulleReservierung) throws DatabaseEntryNotFoundException {
		...
		for (IZimmer zi : alle) {
			...
			{
				List<IReservierung> zimmerreserierungen=new Vector<IReservierung>(zi.getReservierungen());
				for (IReservierung res : zimmerreserierungen) {
					if(res.getVon().after(aktulleReservierung.getBis())||res.getBis().before(aktulleReservierung.getVon()))
					verfügbare.add(zi);
				}
